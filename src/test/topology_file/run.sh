...
TMPROOT=$(mktemp -d "${TMPDIR:-/tmp}/gp_topology_test.XXXXXX")
...
chk "initdb writes a store"            "1"      "$(test -f "$D/gp_topology" && echo 1 || echo 0)"
...
echo "$TOPO3" | $GGTOPO write -D "$D" - >/dev/null
sed -i 's#/d/seg0#/d/segX#' "$D/gp_topology"
$GGTOPO dump -D "$D" >/dev/null 2>&1
chk "a flipped byte fails the checksum" "1" "$?"

echo "$TOPO3" | $GGTOPO write -D "$D" - >/dev/null
head -n -1 "$D/gp_topology" > "$D/x" && mv "$D/x" "$D/gp_topology"
$GGTOPO dump -D "$D" >/dev/null 2>&1
...
chk "no temp file is left behind"      "0" "$(ls "$D" | grep -c 'gp_topology\.tmp')"
...
start_expect "a valid store serves a dispatcher" START -c gp_role=dispatch -c gp_topology_source=file
start_expect "and a utility-mode node"           START -c gp_role=utility  -c gp_topology_source=file

$GGTOPO bootstrap -D "$D" --force >/dev/null
start_expect "generation 0 starts in utility mode"  START  -c gp_role=utility  -c gp_topology_source=file
start_expect "generation 0 refuses a dispatcher"    REFUSE -c gp_role=dispatch -c gp_topology_source=file

echo "$TOPO3" | $GGTOPO write -D "$D" - >/dev/null
sed -i 's#/d/seg0#/d/segX#' "$D/gp_topology"
start_expect "a corrupt store refuses to start"     REFUSE -c gp_role=dispatch -c gp_topology_source=file

echo "$TOPO3" | $GGTOPO write -D "$D" - >/dev/null
rm -f "$D/gp_topology"
start_expect "a deleted store refuses to start"     REFUSE -c gp_role=dispatch -c gp_topology_source=file

# a standby coordinator has to be planted past gg_topology, which refuses it
echo "$TOPO3" | $GGTOPO write -D "$D" - >/dev/null
sed -i 's#^1 -1 p p #1 -1 p m #' "$D/gp_topology"
python3 - "$D/gp_topology" <<'PY' 2>/dev/null || true
import sys, binascii
p = sys.argv[1]
data = open(p, 'rb').read()
...
PY
start_expect "a standby coordinator refuses to start" REFUSE -c gp_role=dispatch -c gp_topology_source=file
...
$POSTGRES -D "$W" -p $PORT -c gp_dbid=1 -c gp_contentid=-1 \
	-c gp_role=utility -c gp_topology_source=file > "$TMPROOT/wr.log" 2>&1 &
WPM=$!
sleep 7
...
chk "no temp files after all that"          "0" "$(ls "$W" | grep -c 'gp_topology\.tmp')"
