...
| **C-4** | `ggdr pause` (immediate pause), then read | Reads work and still answer as of the last **published** restore point — an immediate pause stops the WAL, it does not change what is served. `gg_stat_dr_replica_summary.consistent_paused_point` goes `NULL` because the nodes' *replay* positions no longer agree, while `consistent_restore_point` keeps naming the point still being served — the two columns now say this separately instead of leaving the reader to infer it. | NEW | PASS |
...
| **C-9** | `switch` armed for a restore point production has **not created yet**, **while production churns** (updates, deletes, `ANALYZE`, `VACUUM` of user *and* catalog tables) before creating it | Succeeds when the WAL arrives — nodes catch it cleanly instead of overshooting. **Measured 2026-09-02 (host lab):** as `SELECT gg_dr_switch()` this was **0/8** with churn and 8/8 without — the function's own snapshot was the frozen image, and the first replayed prune cancelled it (`canceling statement due to conflict with recovery`); the earlier AUTO pass only ever ran against an idle production. Now a procedure that holds no snapshot while it waits ([ADR-0009](adr/0009-dr-control-plane-holds-no-snapshot.md)); the base fixture's promote step churns production before creating `dr_rp_promote`, with the switch already armed, and asserts a clean return. | AUTO | PASS |
...
| **C-12** | **Read issued *during* an advance**, with WAL volume skewed to one segment | **Was a reproduced torn read; now fixed and regression-tested.** [The original measurement](#appendix--c-11-measured-a-torn-mid-advance-read-reproduced) caught one distributed transaction half-committed across segments for ~2.1s — a state reachable at neither cut. Every node now answers from the image frozen at the published point regardless of where its replay has got to (ADR-0006 D8), so a mid-advance read returns the *N* image or is cancelled — **for a read that is in flight when replay removes something it needs. A read started afterwards is neither: see C-13.** The fixture holds the skewed state still instead of racing it: the segments are driven to `dr_rp2` by hand while the coordinator stays at `dr_rp1`, and the read must keep returning the `dr_rp1` answer. | AUTO | PASS |

| **C-13** | **Read started *after* replay has pruned a row version the served image needs** (the cluster serves *N*; production updated or deleted the row after *N* and a `VACUUM` or HOT prune of it has replayed; then a fresh read) | **Measured 2026-09-02 (host lab): a wrong answer, silently.** `dr_marker` had one row at the served point; after production updated it eight times and vacuumed, `select count(*)` on the replica returned **0**, with no error, on the coordinator and on each segment in utility mode, and returned 1 again once the next point was published. The old version is gone and the new one is invisible to the frozen image. Hot standby's conflict machinery only cancels snapshots that *exist* when the removal record replays; a frozen image handed out afterwards is unprotected. Contradicts ADR-0005 D2 ("stale, but never wrong") and D6. Candidate fix: mark the served image stale in `ResolveRecoveryConflictWithSnapshot()` when `latestRemovedXid` is at or above its xmin, and refuse reads until the next publish ("the right answer or none" as ADR-0005 states it; per-relation tracking would narrow the refusal). Not implemented — a policy decision ([ADR-0009](adr/0009-dr-control-plane-holds-no-snapshot.md) records it as open). Creating the point *before* running the switch keeps the exposed window to the replay of the WAL between the two points rather than the whole wait. | **RUN** | **GAP (wrong answer)** |
| **C-14** | Two `switch` calls **concurrently**, to two different points ahead | **Measured 2026-09-02:** no mutual exclusion — the last arm wins on every node, one switch completes at its point, the other polls forever and must be cancelled. Nothing corrupted, nothing refused. A mutex in `gg_dr_switch()` would make the second fail fast. | **RUN** | DEGRADE |
| **C-15** | The same restore point **name created twice** on production | **Measured 2026-09-02:** a name created again after the replica is paused at it is a silent no-op (`switch` reports done, replay LSN unchanged); a name that occurs twice *ahead* stops at the first occurrence. Names are the identity; document, do not rely on re-using one. | **RUN** | documented |
...
rewrite halted it. The replica's topology now lives in `$PGDATA/gg_topology`, outside the WAL
...
| **V-21′** | Assert the replica describes **itself**, and is serving that from the file store | `ggdr create` step 7 (`_check_topology()`) reads `gp_segment_configuration` back from the running coordinator — the view over whichever provider is active — and requires it to match the topology file on `dbid`/`content`/`port`/`hostname`; it also requires **every** node to report `gg_topology_source = file`, because that GUC is exempt from the QD/QE sync check and a segment left on `catalog` would read production's replayed rows with nothing else reporting it. The heap-page half of the old check is gone: `ctid` and `pg_relation_size` are heap concepts and mean nothing for a view. | **REWRITTEN (P8)** | PASS |
...
| **G-2** | A WAL segment is **missing** from the archive (retention deleted it, or archiving failed) | That node stalls permanently at the gap. There is no cross-WAN `pg_rewind` — **recovery is a full re-`create` of that node.** Demonstrate the detection path (stall + growing RPO) and the rebuild. | NEW | HALT → rebuild |
...
| **O-2** | `gg_stat_dr_replica_summary` rollup | `consistent_restore_point` non-NULL **only** when every node is *serving* the same point, `consistent_paused_point` non-NULL only when every node has *stopped* at the same one; `rpo_seconds` finite and non-negative. | AUTO | PASS |
...
| **O-4** | Summary while one node lags (mixed restore points) | `consistent_paused_point` is `NULL`, `all_paused = false`, and `consistent_restore_point` still names the published point the cluster answers from. The pair is the signal an operator must trust before serving a report. | NEW | PASS |
| **O-5** | Summary and per-node view **after a subset switch** (`ggdr switch <rp> --content …`), including the case where every node has been re-pointed one at a time | `served_restore_point` = the old point on every node, `restore_point` = the new one, `consistent_restore_point` ≠ `consistent_paused_point`. Reported from the field 2026-08-13: with only the paused point exposed, status named a point no node was serving and called it safe to serve, so the replica looked stuck at the old data. | AUTO (M3 skew) | PASS |
...
replicated catalog and into `$PGDATA/gg_topology`, and P7 deleted the apply-time redo filter,
...
| DR catalog after `create` | 12 pages, 3 live rows, **highest live block 11** |
